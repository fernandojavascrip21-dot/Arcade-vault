...
- Nuevo asset `public/games/rompemuros/spritesheet-breakout.png` (copia de `references/started-games/04-arkanoid/assets/spritesheet-breakout.png`) — primer asset de imagen del catálogo de juegos portados. El motor lo carga con `new Image()` apuntando a `/games/rompemuros/spritesheet-breakout.png` antes de arrancar el loop (mismo patrón `loadSpritesheet(cb)` que el original, adaptado sin variables de módulo globales fuera de la factoría).
...
- Cambios en `app/play/[id]/play-room.tsx`, solo activos cuando `game.id === "rompemuros"` (bandera `isRompemuros`, análoga a `isAsteroids`/`isBloques`):
  ...
  - El HUD externo (PUNTUACIÓN, VIDAS, NIVEL) ya se muestra por defecto para cualquier juego que no sea `isBloques` (la condición actual es `{!isBloques ? (...) : null}`), así que no requiere ningún cambio adicional para mostrarse también en Rompemuros — se conecta a los valores reales vía `onStateChange`.
...
- [ ] El HUD externo de `PlayRoom` (PUNTUACIÓN, VIDAS, NIVEL) refleja en vivo el estado real del juego, en paralelo al HUD dibujado dentro del canvas (Score arriba-izquierda, vidas como íconos de pelota arriba-derecha) — ambos muestran la misma información al mismo tiempo.
...
- **Sí:** mantener también el overlay "PAUSADO" propio del canvas, en paralelo al overlay "EN PAUSA" de React (dual), en vez de apagar el interno. Motivo: decisión explícita del usuario — mismo criterio del HUD doble que ya estableció Asteroides (spec 06): redundancia intencional entre el canvas original y la capa informativa de React.
- **Sí:** HUD doble (Score y vidas dibujados dentro del canvas, y en paralelo reflejados en el HUD externo de `PlayRoom`), igual que Asteroides, en vez de ocultar el HUD externo como hace Bloques. Motivo: decisión explícita del usuario — Arkanoid no tiene un panel lateral grande como el de Bloques (con NEXT), así que no hay riesgo de amontonamiento visual al duplicar los valores.
...
| El canvas ampliado con dos overlays de pausa (el del motor y el de React) podría verse recargado si ambos difieren mucho en estilo.                                                                                                                      | Aceptado como compromiso explícito (sección 6, mismo criterio que el HUD doble de Asteroides): la redundancia es intencional, y el overlay del motor conserva el estilo simple del original (texto blanco sobre fondo semitransparente).    |
